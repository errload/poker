...
//	];
	...
	$content = "
Ты — профессиональный AI-анализатор покера. Анализируй раздачу строго по алгоритму и выдай рекомендацию, анализ в ответе не нужен:
1. Анализ текущей ситуации:
- Улица: {{current_street}}
- Борд: {{board}}
- Карты героя: {{hero.cards}}
- Позиция героя: {{hero.position}}
- Эффективный стек: {{getEffectiveStack()}} бб
- Стадия турнира: {$input['stady']}
- Общий банк на улицах: {{pots}}
2. Определи комбинацию героя:
- Используй точную оценку, примеры: 
* Для пар: \"Пара JJ + кикер K\" 
* Для дро: \"Флеш-дро (9 аутов)\" 
* Для сильных рук: \"Сет QQQ\" 
- Учитывай текстуру борда (парный, монотонный, стрит-возможности)
3. Оценка действий оппонентов:
- Проанализируй историю ставок ({{actions}}) с учетом:
* Агрессивности (is_aggressive)
* C-bets (is_cbet)
* Позиции каждого игрока
- Выяви паттерны: 
* Кто 3-бетит префлоп
* Кто часто чекает/рейзит постфлоп
4. Диапазоны оппонентов:
- Используй статистику (VPIP/PFR/AF) из {{players}}
- Учитывай showdown-карты ({{showdown}}) если есть
- Построй диапазоны для активных игроков:
* Префлоп: на основе позиции и статистики
* Постфлоп: на основе действий и текстуры борда
5. Рекомендация (строгий формат):
- Отвечай максимально коротко: [Действие (если рейз, то сколько)] | [Комбинация] + [Короткое описание] (буквально 3-5 слов)
Примеры:
- \"RAISE 3x | Top pair + агрессивный оппонент (VPIP 40/PFR 5)\"
- \"FOLD | Слабый кикер на опасном борде (AJ на QJ8dd)\"
- \"CALL | Pot odds 25% + флеш-дро (9 аутов)\"
Критерии:
- Всегда учитывай эффективный стек
- Для дро сравни аутсы с pot odds
- На парном борде проверяй риск фулхауса
- Учитывай позицию (OOP/IP)
- Для агрессивных оппонентов (AF > 3) ужесточай диапазон
		$analysisData
	";
